...
#
# Два проекта compose из одного файла, blue и green. Новая версия поднимается в
# свободном цвете рядом с работающим, и только после подтверждения от прокси,
# что он балансирует на новый экземпляр, гасится прежний.
...
# --wait ждёт HEALTHCHECK самого образа: он ходит в /health внутри контейнера
