...
public class DialogflowService : IDialogflowService
{
    ...
    private readonly Dictionary<InternalModels.Source, Func<InternalModels.Request, string, EventInput>>
        _eventResolvers;
    ...
    private async Task<InternalModels.Dialog> GetResponseInternalAsync(InternalModels.Request request,
        SessionsClient client, ScopeContext context)
    {
               ...
               }, "Get response from Dialogflow"))
        {
            ...
            context.TryGetParameterValue(nameof(DialogflowConfiguration.ProjectId), out var projectId);
            context.TryGetParameterValue(nameof(DialogflowConfiguration.LanguageCode), out var languageCode);
            context.TryGetParameterValue(nameof(DialogflowConfiguration.Region), out var region);
            context.TryGetParameterValue(nameof(DialogflowConfiguration.LogQuery), out var logQuery);
            ...
            if (isLogQuery)
                _log.LogTrace($"Request:{Environment.NewLine}{intentRequest.Serialize()}");

            var intentResponse = await client.DetectIntentAsync(intentRequest).ConfigureAwait(false);

            if (isLogQuery)
                _log.LogTrace($"Response:{Environment.NewLine}{intentResponse.Serialize()}");
    ...
    private Task SetContextInternalAsync(ContextsClient client, string sessionId, ScopeContext scopeContext,
        string contextName, int lifeSpan = 1, IDictionary<string, string> parameters = null)
    {
        using (Tracing.Trace())
        {
            scopeContext.TryGetParameterValue(nameof(DialogflowConfiguration.ProjectId), out var projectId);
            scopeContext.TryGetParameterValue(nameof(DialogflowConfiguration.Region), out var region);
        ...
        string region)
    {
        using (Tracing.Trace())
        {
            ...
            if (text?.Length > MaximumRequestTextLength) text = request.Text.Substring(0, MaximumRequestTextLength);
            ...
            if (eventInput != null) query.Event = eventInput;
    ...
    private EventInput EventByCommand(string requestText, string languageCode)
    {
        using (Tracing.Trace())
        {
            ...
            if (!string.IsNullOrEmpty(eventName)) result = GetEvent(eventName, languageCode);
    ...
    {
        if (string.IsNullOrWhiteSpace(locationId)) return SessionName.FromProjectSession(projectId, sessionId);
