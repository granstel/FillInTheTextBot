    ...
    : IConversationService
{
    ...
    public async Task<Response> GetResponseAsync(Request request)
    {
        ...
        if (isGotResetParameter &&
            string.Equals(resetTextIndex, bool.TrueString, StringComparison.InvariantCultureIgnoreCase))
        ...
        if (string.Equals(dialog?.Action, "CALL_RATING")) response.Text = "CALL_RATING";
    ...
    private Texts TryAddReplacementsFromPayload(Payload payload, Source source, string text)
    {
        ...
        if (payload is null) return texts;
        ...
        if (replacements?.Any() != true) return texts;
    ...
    private IDictionary<string, string> GetEmotions(Dialog dialog)
    {
        using (Tracing.Trace())
        {
            ...
            if (result.Any() || dialog?.ParametersIncomplete == true || string.IsNullOrEmpty(textName)) return result;
    ...
    private Dictionary<string, string> GetEmotionsFromDialog(Dialog dialog)
    {
        using (Tracing.Trace())
        {
            ...
            foreach (var emotionKey in EmotionsKeysMap.SourceEmotionKeys.Values)
            {
                ...
                if (!string.IsNullOrEmpty(emotion)) result.Add(emotionKey, emotion);
            }
    ...
    {
        if (appeal != Appeal.Official) return responseText;
    ...
    private ICollection<Button> GetButtonsFromPayload(ICollection<Button> responseButtons, Payload dialogPayload,
        Source requestSource)
    {
        using (Tracing.Trace())
        {
            ...
            if (responseButtons?.Any() == true) buttons.AddRange(responseButtons);
    ...
    {
        if (dialog?.ParametersIncomplete != true && string.Equals(dialog?.Action ?? string.Empty, "saveToRepeat",
    ...
    {
        if (isCancelsSlotFilling is not true) return response;
